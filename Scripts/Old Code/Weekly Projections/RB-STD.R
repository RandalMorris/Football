...
#URLS ####
url_cbs <- paste("http://www.cbssports.com/fantasy/football/stats/weeklyprojections/RB/", week, "/avg?&print_rows=9999", sep="")
...
url_fantasy_pro_ranks <- ("http://www.fantasypros.com/nfl/rankings/rb.php")
url_fantasy_pro <- ("http://www.fantasypros.com/nfl/projections/rb.php")
url_numfire <- paste("http://www.numberfire.com/nfl/fantasy/fantasy-football-projections/rb")
...
cbs_rbproj$cbs_fpts <- cbs_rbproj$cbs_fpts + (cbs_rbproj$cbs_rushyd/10) + (cbs_rbproj$cbs_rushtd*6) + (cbs_rbproj$cbs_recyd/10) + (cbs_rbproj$cbs_rectd*6) + (cbs_rbproj$cbs_fl*-2)
...
#name the columns
names(fftoday_rbproj) <- names(fftoday_rbproj1) <- c("player","fft_team","fft_rushatt","fft_rushyd","fft_rushtd",
                                                     "fft_recpts","fft_recyd","fft_rectd","fft_fpts")
#Merge and remove other dataframes to reduce clutter
fftoday_rbproj <- rbind(fftoday_rbproj, fftoday_rbproj1)
rm(fftoday_rbproj1)
#Convert to numeric
fftoday_rbproj$fft_rushatt <- as.numeric(fftoday_rbproj$fft_rushatt)
fftoday_rbproj$fft_rushyd <- as.numeric(fftoday_rbproj$fft_rushyd)
fftoday_rbproj$fft_rushtd <- as.numeric(fftoday_rbproj$fft_rushtd)
fftoday_rbproj$fft_recpts <- as.numeric(fftoday_rbproj$fft_recpts)
fftoday_rbproj$fft_recyd <- as.numeric(fftoday_rbproj$fft_recyd)
fftoday_rbproj$fft_rectd <- as.numeric(fftoday_rbproj$fft_rectd)
...
#Remove all insginicant projections
fftoday_rbproj1 <- subset(fftoday_rbproj, fftoday_rbproj$fft_fpts > 1.99)
fftoday_rbproj <- fftoday_rbproj1
rm(fftoday_rbproj1)
#resort and order by name
fftoday_rbproj <- subset(fftoday_rbproj, select = c("name","fft_team","fft_rushatt","fft_rushyd","fft_rushtd",
                                                    "fft_recpts","fft_recyd","fft_rectd","fft_fpts"))
...
##MERGE DATA##
projections <- merge(cbs_rbproj, espn_rbproj, by="name", all.x = TRUE)
projections <- merge(projections, fftoday_rbproj, by="name", all.x = TRUE)
projections <- merge(projections, ffs_rbproj, by="name", all.x = TRUE)
...
ggplot(fantasyproj, aes(x=mean, y=rank, color=factor(Tier))) +
  ...
  labs(title = paste("Week ", week , " Standard Running Back Projections", sep="")) +
  coord_cartesian(xlim =c(0,(max(fantasyproj$mean)+10))) 
ggsave(paste(getwd(),"/Figures/Historical Figures/Weekly/STD/RB/Week ",week," Running Back.jpg", sep=""), width=10, height=10)
ggsave(paste(getwd(),"/Figures/Weekly/STD/RB/Week ",week," Running Back.jpg", sep=""), width=10, height=10)

#Save file
save(fantasyproj, file = paste(getwd(),"/Data/Weekly/STD/RB/Week",week,"-fantasyproj.RData", sep=""))
write.csv(fantasyproj, file=paste(getwd(),"/Data/Weekly/STD/RB/Week",week,"-fantasyproj.csv", sep=""), row.names=FALSE)

save(projections, file = paste(getwd(),"/Data/Weekly/STD/RB/Week",week,"-projections.RData", sep=""))
write.csv(projections, file=paste(getwd(),"/Data/Weekly/STD/RB/Week",week,"-projections.csv", sep=""), row.names=FALSE)

save(fantasyproj, file = paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week",week,"-fantasyproj.RData", sep=""))
write.csv(fantasyproj, file=paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week",week,"-fantasyproj.csv", sep=""), row.names=FALSE)

save(projections, file = paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week",week,"-projections.RData", sep=""))
write.csv(projections, file=paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week",week,"-projections.csv", sep=""), row.names=FALSE)
...
#save the html file
print.xtable(htmltable_fantasyproj, type="html", file=paste(getwd(),"/Data/Weekly/STD/RB/Week ",week," tier.html",sep=""))
print.xtable(htmltable_projections, type="html", file=paste(getwd(),"/Data/Weekly/STD/RB/Week ",week," rbprojections.html",sep=""))

print.xtable(htmltable_fantasyproj, type="html", file=paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week ",week," tier.html",sep=""))
print.xtable(htmltable_projections, type="html", file=paste(getwd(),"/Data/Historical Projections/Weekly/STD/RB/Week ",week," rbprojections.html",sep=""))
