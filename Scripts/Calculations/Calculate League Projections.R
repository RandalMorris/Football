...
playerNames <- melt(projections,
                    id.vars = c("name","pos","team"),
                    measure.vars = paste("name", sourcesOfProjectionsAbbreviation, sep="_"),
                    na.rm=TRUE,
                    value.name="name")
playerNames=as.data.table(playerNames)
setkeyv(playerNames, cols=c("name","pos","team"))
...
teamNames <- melt(projections,
                    ...
                    na.rm=TRUE,
                    value.name="team")
teamNames=as.data.table(teamNames)
setkeyv(teamNames, cols=c("name","pos","team"))
...
projectionsAvg <- projections[, lapply(.SD, mean, na.rm=TRUE), by=c("name","pos","team"), .SDcols=availableVars]
projectionsAvg$sourceName <- "average"

#Calculate Hodges-Lehmann (pseudo-median) robust average of categories
projectionsRobustAvg <- projections[, lapply(.SD, function(x) tryCatch(wilcox.test(x, conf.int=TRUE, na.action="na.exclude")$estimate, error=function(e) median(x, na.rm=TRUE))), by=c("name","pos","team","team"), .SDcols=availableVars]
...
for(col in availableVars){
  projections[is.na(get(col)) & (sourceName == "average" | sourceName == "averageRobust"), (col) := 0]
}
...
#projections[,fumblesPts := paste(fumbles,suffix, sep="_") * fumlMultiplier]
...
projections[-which(sourceName %in% c("average","averageRobust")), pointsLo := tryCatch(wilcox.test(points, conf.int=TRUE, na.action="na.exclude")$conf.int[1], error=function(e) median(points, na.rm=TRUE)), by=c("name","pos","team","playerID")]
projections[-which(sourceName %in% c("average","averageRobust")), pointsHi := tryCatch(wilcox.test(points, conf.int=TRUE, na.action="na.exclude")$conf.int[2], error=function(e) median(points, na.rm=TRUE)), by=c("name","pos","team","playerID")]

projections[,pointsLo := mean(pointsLo, na.rm=TRUE), by=c("name","pos","team","playerID")]
projections[,pointsHi := mean(pointsHi, na.rm=TRUE), by=c("name","pos","team","playerID")]
