...
cases <- projections[, c("name","pos", paste("team", sourcesOfProjectionsAbbreviation, sep="_")), with=FALSE]
uniqueCases <- unique(projections[, c("name","pos",paste("team", sourcesOfProjectionsAbbreviation, sep="_")), with=FALSE])
duplicateCases <- uniqueCases[duplicated(name) | duplicated(name, fromLast=TRUE)]

#Different player (Same name, different team)
setkeyv(duplicateCases, c("name", paste("team", sourcesOfProjectionsAbbreviation, sep="_")))
...
projectionsAvg <- projections[, lapply(.SD, mean, na.rm=TRUE), by=c("name","pos"), .SDcols=availableVars]
projectionsAvg$sourceName <- "average"

#Calculate Weighted Average
setkeyv(projections, cols=c("name","pos","sourceName"))
projectionsAllSources <- projections #if error, check for duplicate cases: table(projectionsAllSources$name)[table(projectionsAllSources$name) != length(unique(projections$sourceName))]
...
setkeyv(projectionsAllSources, cols=c("name","pos","sourceName"))
projectionsWeightedAvg <- projectionsAllSources[, lapply(.SD, function(x) weighted.mean(x, weights, na.rm=TRUE)), by=c("name","pos"), .SDcols=availableVars]

projectionsWeightedAvg$sourceName <- "averageWeighted"

#Merge
projectionCalculations <- rbind(projectionsAvg, fill=TRUE)

projections <- rbind(projections, projectionCalculations, fill=TRUE)

#Set key
setkeyv(projections, cols=c("name","pos"))
projections[, playerID := (.GRP), by=c("name","pos")]
